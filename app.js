...
async function loadSessions() {
  ...
  if (!tbody) return;
  tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:24px;color:var(--text-muted);">جاري التحميل...</td></tr>`;

  try {
    const snap = await db.collection("user_sessions").orderBy("updatedAt", "desc").get();
    sessionsCache = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    calculateSessionStats();
    applySessionFilters();
  } catch (err) {
    tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:24px;color:var(--danger);">خطأ في التحميل: ${err.message}</td></tr>`;
  }
}

function calculateSessionStats() {
  const total = sessionsCache.length;
  const active = sessionsCache.filter(s => s.activeSession).length;
  const withToken = sessionsCache.filter(s => s.deviceToken).length;
  const inactive = total - active;

  const el = id => document.getElementById(id);
  if (el("sessionsTotal")) el("sessionsTotal").textContent = total;
  if (el("sessionsActive")) el("sessionsActive").textContent = active;
  if (el("sessionsWithToken")) el("sessionsWithToken").textContent = withToken;
  if (el("sessionsInactive")) el("sessionsInactive").textContent = inactive;
}
...
function applySessionFilters() {
  ...
  let list = sessionsCache;
  if (query) list = list.filter(s => (s.userId ?? "").toLowerCase().includes(query));
  if (filter === "active") list = list.filter(s => s.activeSession);
  if (filter === "inactive") list = list.filter(s => !s.activeSession);
...
function renderSessions(sessions) {
  ...
  if (!sessions.length) {
    tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:24px;color:var(--text-muted);">لا توجد جلسات مسجلة</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = sessions.map(s => {
    ...
    const badgeLabel = isActive ? "نشط الآن" : "غير نشط";
    const platform = s.platform ?? "—";
    const loginAt = s.lastLoginAt ? new Date(s.lastLoginAt).toLocaleString("ar-EG") : "—";
    const logoutAt = s.lastLogoutAt ? new Date(s.lastLogoutAt).toLocaleString("ar-EG") : "—";

    let tokenCell;
    if (s.deviceToken) {
      const short = s.deviceToken.substring(0, 20) + "...";
      tokenCell = `<span class="token-chip" title="${s.deviceToken}">${short}</span>`;
    } else {
      tokenCell = `<span class="no-token">لا يوجد رمز</span>`;
    }

    return `
      <tr>
        <td><strong>${s.userId ?? s.id}</strong></td>
        <td>
          <span class="session-badge ${badgeClass}">
            <span class="dot"></span>${badgeLabel}
          </span>
        </td>
        <td style="font-size:12px;color:var(--text-muted);">${platform}</td>
        <td style="font-size:12px;">${loginAt}</td>
        <td style="font-size:12px;color:var(--text-muted);">${logoutAt}</td>
        <td>${tokenCell}</td>
      </tr>`;
  }).join("");
}

// Wire up sessions search + filter
document.addEventListener("DOMContentLoaded", () => {
  const searchEl = document.getElementById("sessionSearch");
  const filterEl = document.getElementById("sessionFilter");
  if (searchEl) searchEl.addEventListener("input", applySessionFilters);
  if (filterEl) filterEl.addEventListener("change", applySessionFilters);
});
